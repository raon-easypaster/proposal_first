...
2. **레이아웃 (Compact Mode)**:
   ...
   - 불필요한 서론이나 인사말은 생략하고 즉시 본론으로 들어가십시오.
3. **시각적 요소 (인포그래픽 필수 포함)**: 
   - 각 주요 챕터(문제분석, 사업내용, 기대효과 등)마다 내용을 요약하는 **[인포그래픽 제안]** 박스를 반드시 삽입하십시오.
   - 양식: 
     > 🖼️ **[인포그래픽 제안] (제목)**
     > - (구성 요소 및 흐름 설명)
4. **평가 계획 (필수)**:
...
### 1) 사업의 필요성
- 대상자의 욕구 및 문제점 (구체적 통계 인용)
- 지역사회 환경적 특성 및 시급성
- **> 🖼️ [인포그래픽 제안] 문제 분석 트리(Problem Tree)**

### 2) 서비스 지역 및 대상자 선정
- 선정 기준 및 산출 근거 (일반집단 > 위기집단 > 표적집단 > 실인원)
- **표(Table)** 형태로 정리하여 제시

### 3) 사업 목적 및 목표
- 산출목표(Output)와 성과목표(Outcome) 구분
- SMART 기법 적용

### 4) 사업 내용 (세부 프로그램)
- 프로그램명, 수행기간, 수행인력, 수행방법, 세부 진행내용
- **> 🖼️ [인포그래픽 제안] 프로그램 운영 프로세스 맵**
...
### 6) 평가 계획 (필수 사항)
- **반드시 아래 양식의 표(Table)로 작성하십시오:**
  | 평가 지표 (성과목표) | 측정도구 | 평가 방법 | 평가 시기 |
  |---|---|---|---|
  | (내용) | (척도명 등) | (사전사후 비교 등) | (시기) |

### 7) 기대 효과
- 참여자 변화 및 지역사회 변화
- **> 🖼️ [인포그래픽 제안] 사업 전후 변화 비교도**
`.trim();
