...
[⚠️ 절대적 작성 원칙 - 반드시 준수할 것]
1. **서술 태도**: 신뢰감을 주는 전문적인 용어와 명료한 종결어미(~함, ~임, ~이어야 함)를 사용하십시오.
2. **레이아웃 (초고밀도 모드)**:
   - **빈 줄 절대 금지**: 문단과 문단 사이, 리스트 항목 사이에 빈 줄을 넣지 마십시오. 정보의 밀도를 극대화하여 빽빽하게 작성하십시오.
   - 불필요한 서론, 인사말, 마무리 인사는 **절대 생략**하고 즉시 목차 1번부터 시작하십시오.
3. **시각적 요소 (인포그래픽 필수 포함)**: 
   - 각 주요 챕터(문제분석, 사업내용, 기대효과) 하단에 내용을 도식화할 수 있는 **[인포그래픽 아이디어]** 박스를 반드시 삽입하십시오.
   - 양식: 
     > 🖼️ **[인포그래픽 제안] (제목)**
     > - (구성 요소 및 흐름 설명)
4. **평가 계획 (필수 표)**:
   - 성과지표와 평가방법은 반드시 **마크다운 표(Table)** 형태로 작성하십시오.
...
`;
    ...
    if (attachedFile) {
      prompt += `
[참고 자료 분석 요청]
첨부된 PDF 파일(사업 공고문 또는 관련 자료)을 정밀 분석하여, 해당 공모전의 취지와 심사 기준을 사업계획서 내용에 완벽히 녹여내십시오.
---
`;
    }
...
## 3. 상세 작성 요청 항목 (목차)
**모든 항목은 빈 줄 없이 빽빽하게 작성하십시오.**

### 1) 사업의 필요성 (문제 분석)
- 대상자의 욕구 및 문제점 (구체적 통계 및 수치 인용 필수)
- 지역사회 환경적 특성 및 사업의 시급성
- 기존 유사 사업과의 차별성
- **> 🖼️ [인포그래픽 제안] 문제 원인 분석도 (Problem Tree)**

### 2) 서비스 지역 및 대상자 선정
- 선정 기준 및 산출 근거를 **표(Table)** 형태로 요약 제시 (일반집단 > 위기집단 > 표적집단 > 실인원)
- 대상자 선정의 타당성

### 3) 사업 목적 및 목표
- 산출목표(Output)와 성과목표(Outcome) 구분 기술
- 목표는 SMART(구체적, 측정가능, 달성가능, 관련성, 시간제한) 원칙 준수

### 4) 사업 내용 (세부 프로그램)
- 프로그램명, 수행기간, 수행인력, 수행방법, 세부 진행내용을 상세히 기술
- **> 🖼️ [인포그래픽 제안] 프로그램 운영 프로세스 맵 (Flow Chart)**

### 5) 예산 계획
- 산출 내역 상세 기재 (산출식 포함: 단가 x 인원 x 횟수)
- 비목별(사업비, 운영비 등) 구분하여 **표(Table)**로 작성

### 6) 평가 계획 (필수 사항)
- **반드시 아래 양식의 표(Table)로만 작성하십시오:**
  | 성과목표 | 성과지표 | 측정도구 | 평가방법 | 평가시기 |
  |---|---|---|---|---|
  | (내용) | (내용) | (척도명 등) | (사전사후 등) | (시기) |

### 7) 기대 효과
- 참여자(개인) 변화
- 지역사회 변화 및 파급 효과
- **> 🖼️ [인포그래픽 제안] 사업 전후 변화 비교도 (Before & After)**
`.trim();
